...
var (
	...
	input = flag.String("input", "kinglear.txt", "File(s) to read.")
...
func main() {
	...
	windowedLines := beam.WindowInto(s, window.NewFixedWindows(time.Minute), window.NewAlwaysTrigger(), timestampedLines)
	...
	merged := beam.WindowInto(s, window.NewGlobalWindows(), window.NewNeverTrigger(), formatted)
